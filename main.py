...
class User(UserMixin, db.Model):
    ...
    # Similarly we are making a comment section that will be attached to each user
    # The "commenter" will refer to the commenter in the Comment class
...
class Comment(db.Model):
    ...
    # Below we are creating the child relationship
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("There isn't an account associated with that email.")
            return redirect(url_for('login'))
        elif not check_password_hash(pwhash=user.password, password=password):
            flash("Sorry, the password you entered is incorrect. Please try again.")
            return redirect(url_for('login'))
